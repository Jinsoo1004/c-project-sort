...
void selectionSort(int *value, int n) {
    ...
    for (i = 0; i < n - 1; i++) {
        ...
        // ฐหป็วั ฐชม฿ ฐกภๅ ภÛภบ ฐชฐ๚ value[i]ฐช swap ฐ๚มค.
        temp1 = value[i];
        value[i] = value[min];
        value[min] = temp1;
    }
...
void insertion_sort(int list[], int n) {
    ...
    for (i = 1; i < n; i++) {
        key = list[i]; // ว๖ภ็ ป๐ภิตษ ผýภฺภฮ iน๘ยฐ มคผ๖ธฆ key บฏผ๖ทฮ บนป็
...
void quick_sort(int *data, int start, int end) {
    ...
    int pivot = start;
    int i = pivot + 1; // ฟÞยส รโน฿ ม๖มก
    int j = end; // ฟภธฅยส รโน฿ ม๖มก
    int temp;

    while (i <= j) {
        // ฦ๗ภฮลอฐก พ๙ฐฅธฑถงฑ๎ม๖ นÝบน
...
void heapify(int *arr, int size){
    for(int i=1;i<size;++i){
        ...
        do{
            ...
            child = root;
        }while(child!=0);	//รึป๓ดÜ ณ๋ตๅฑ๎ม๖ มกฐห
    }
}

//รึป๓ดÜ ณ๋ตๅฟอ รึวฯดÜ ณ๋ตๅ ฑณรผ
...
int main() {
    ...
    switch (input) {
        case 1:
            ...
            printf("\n");
            printf("ปýผบตศ ทฃดýฐช : ");
            for (int i = 0; i < 25; i++) { // 25น๘ นÝบน
                value[i] = rand() % 99 + 1; // ณญผ๖ ปýผบ
...
//            int list[25] = {8, 5, 6, 2, 4, 5, 23, 3,32 ,35,235,235, 25,53,2,243, 5,62,3235, 523,2,23 ,5,8,5};
        ...
        case 4:
            printf("\nฤüมคทฤภป ผฑลรวฯผฬฝภดฯดู\n");
            int data[25];
            printf("\n");
            printf("ปýผบตศ ทฃดýฐช : ");
            for (int i = 0; i < 25; i++) { // 25น๘ นÝบน
                data[i] = rand() % 99 + 1; // ณญผ๖ ปýผบ
